    ...
    v-chakra="(theme) => ({
      ...
      '.algolia-autocomplete .ds-dropdown-menu': {
        ...
        '::before': {
          ...
          borderColor: theme.colors.vue[400],
          backgroundColor: `${theme.colors.vue[400]} !important`
        }
      },
  ...
  >
    <CInputGroup>
      ...
      <CInput
        ...
        ref="docsearchInput"
        w="400px"
        focus-border-color="vue.400"
        variant="filled"
        type="search"
        placeholder="Search nft. (Press '/' to focus)"
      />
    </CInputGroup>
  </CFlex>
</template>
...
<script>
import { CInput, CFlex, CIcon, CInputGroup, CInputLeftElement } from '@chakra-ui/vue'
...
export default {
  ...
  async mounted () {
    ...
    const startsWithCss = str => startsWith('css-', str)
    ...
    docsearch({
      apiKey: '34eb78385ea9aa8f65d255cce8f1b962',
      indexName: 'chakra-ui_vue',
      ...
      handleSelected: (input, event, suggestion) => {
        ...
        input.setVal('')
        input.close()
        const docsearchInput = _this.$refs.docsearchInput.$el
        if (docsearchInput) {
          docsearchInput.blur()
        }

        const url = suggestion.url.replace('https://vue.chakra-ui.com', '')
  ...
  methods: {
    async handleKeydown (e) {
      await this.$nextTick()
      const docsearchInput = this.$refs.docsearchInput.$el
      if (document.activeElement !== docsearchInput) {
        lastActiveElement = document.activeElement
      }

      if (e.key === '/') {
        docsearchInput && docsearchInput.focus()
      } else if (
        e.key === 'Escape' &&
          document.activeElement === docsearchInput
      ) {
